...
public class BookingFlowApiController {
        ...
        public ResponseEntity<?> getCinemasByMovie(@PathVariable Integer movieId) {
                ...
                return ResponseEntity.ok(
                                rows.stream().map(r -> Map.of("id", r[0], "name", r[1])).collect(Collectors.toList()));
        }
        ...
        @GetMapping("/showtimes/{roomId}/{movieId}")
        public ResponseEntity<?> getShowtimes(
                        @PathVariable Integer roomId,
                        @PathVariable Integer movieId) {

                String sql = """
                                    SELECT MaSuatChieu, NgayChieu, GioBatDau
                                    FROM SUATCHIEU
                                    WHERE MaPhong = ? AND MaPhim = ?
                                """;
                ...
                return ResponseEntity.ok(
                                rows.stream().map(r -> Map.of(
                                                "id", r[0],
                                                "label", String.valueOf(r[1]) + " " + String.valueOf(r[2]))).toList());
        }
}
